...
uses
  ...
  Vcl.ComCtrls, ShlObj, ActiveX, ComObj;
...
type
  ...
  TForm2 = class(TForm)
    ...
    Stage5GameTimer: TTimer;
    Stage5Task1Label: TLabel;
    Stage5GameWordsLabel: TLabel;
    Stage5ResultLabel: TLabel;
    Stage5ConfirmWordsButton: TButton;
    Stage5MemoForWords: TMemo;
    ...
    procedure Stage5MemoForWordsChange(Sender: TObject);
    procedure Stage5MemoForWordsKeyPress(Sender: TObject; var Key: Char);
    procedure Stage5GameTimerTimer(Sender: TObject);
    procedure Stage5ConfirmWordsButtonClick(Sender: TObject);
...
Procedure GetGameWordsFromFileStage_2_3_4_5(Path: String);
Var
  InputFile: TextFile;
  IsCorrect: Boolean;
  List: TStringList;
...
Function CompareWordsStage1(PlayerWord: String; Path:String): Boolean;
Var
  I, J: Integer;
...
Begin
  ...
  if (PlayerWord = ReversedGameWord) then
    CompareWordsStage1 := True
  else
    CompareWordsStage1 := False;
End;
...
Begin
  ...
  if CorrectWordsCounter = High(Unit2.GameWords) + 1 then
    IsWordsCorrect := True
  else
...
Begin
  CorrectWordsCounter := 0;
  SetLength(ReversedGameWords, Length(Unit2.GameWords));
  ...
  if CorrectWordsCounter = High(ReversedGameWords) + 1 then
    IsWordsCorrect := True
  else
    IsWordsCorrect := False;
...
Function CompareWordsStage5(PlayerWords: TArrStr; Path: String): Boolean;
Var
  I, J, K: Integer;
  ReversedGameWords: TArrStr;
  IsWordsCorrect: Boolean;
...
Begin
  ...
  for I := Low(ReversedGameWords) to High(ReversedGameWords) do
  Begin
    GameStr := GameStr + ReversedGameWords[I] ;
    PlayerStr := PlayerStr + PlayerWords[I];
  End;
  ...
  CompareWordsStage5 := IsWordsCorrect;
End;
...
Const
  MESSAGE_1 = 'Данная игра предназначена для тренировки памяти игрока.' + #13#10;
  MESSAGE_2 = 'В процессе игры на экран буду выводиться слова и через некоторое время пропадать.' + #13#10;
  MESSAGE_3 = 'Задачей игрока является запоминать эти слова и вводить эти же слова, но в обратном порядке.' + #13#10;
  MESSAGE_4 = 'Со временем игра начинает усложняться: слова становятся длиннее. На их запоминание дается чуть больше времени.' + #13#10;
  MESSAGE_5 = 'Игра заканчивается, если игрок правильно введет три перевернутых слова из 8 букв.' + #13#10;
  MESSAGE_6 = 'ВАЖНО! Игрок должен подключить пять файлов:' + #13#10;
  MESSAGE_7 = '1-ый файл содержит слова из 5-ти букв;' + #13#10;
  MESSAGE_8 = '2-ой файл содержит слова из 6-ти букв;' + #13#10;
  MESSAGE_9 = '3-ий файл содержит слова из 7-ти букв;' + #13#10;
  MESSAGE_10 = '4-ый файл содержит слова из 8-ти букв;' + #13#10;
  MESSAGE_11 = '5-ый файл содержит список слов длинной от 5-ти до 8-ми букв(слова могут отличаться от слов из 1-4 файлов):';
  MESSAGE_12 = 'Игра берёт слова из списка,который игрок может подключить(свои файлы со словами).'+ #13#10;
  MESSAGE_13 = 'Удачи и успехов!';
begin
  Application.MessageBox(MESSAGE_1 + MESSAGE_2 + MESSAGE_3 + MESSAGE_4 + MESSAGE_5 +
  MESSAGE_6 + MESSAGE_7 + MESSAGE_8 + MESSAGE_9 + MESSAGE_10 + MESSAGE_11 +
  MESSAGE_12 + MESSAGE_13, 'Правила игры');
end;
...
begin
  ...
  IsWordCorrect := CompareWordsStage1(PlayerWord, Path);
...
begin
  ...
  for I := Low(Unit2.GameWords) to High(Unit2.GameWords) do
  Begin
    PlayerWords[I] := Form2.Stage4MemoForWords.Lines[I];
  End;
...
procedure TForm2.Stage5ConfirmWordsButtonClick(Sender: TObject);
Var
  IsWordsCorrect: Boolean;
  PlayerWords: TArrStr;
  I: Integer;
begin
  Stage5ConfirmWordsButton.Enabled := False;
  SetLength(PlayerWords, Length(Unit2.GameWords));

  for I := Low(Unit2.GameWords) to High(Unit2.GameWords) do
  Begin
    PlayerWords[I] := Form2.Stage5MemoForWords.Lines[I];
  End;

  IsWordsCorrect := CompareWordsStage5(PlayerWords, Path);
  if IsWordsCorrect then
  Begin
    Stage5ResultLabel.Visible := True;
    Stage5ResultLabel.Font.Color := clGreen;
    Stage5ResultLabel.Left := 320;
    Stage5ResultLabel.Caption := 'Правильный ответ!';
  ...
  Begin
    Stage5ResultLabel.Visible := True;
    Stage5ResultLabel.Font.Color := clRed;
    Stage5ResultLabel.Left := 320;
    Stage5ResultLabel.Caption := 'Неправильный ответ!';
  End;

  Delay(500);
  Form2.Stage5MemoForWords.Clear;
  Stage5ResultLabel.Caption := '';
  Stage5ResultLabel.Left := 288;

  Stage5GameTimer.Enabled := True;
end;
...
begin
  ...
  Begin
    ...
    GetGameWordsFromFileStage_2_3_4_5(Unit2.Path);
...
begin
  ...
  Begin
    ...
    Application.MessageBox('Поздравляем с победой! Вы прошли четвертый этап. Переходим к следующему', 'Победа в четвертом этапе!');

    Unit2.StageCounter := 5;
    Unit2.PhaseCounter := 1;
    FourthStage.TabVisible := False;
    FourthStage.Enabled := False;
    FifthStage.TabVisible := True;
    FifthStage.Enabled := True;
    Stage5ConfirmWordsButton.Default := True;
    Stage5GameTimer.Enabled := True;
    Stage5GameWordsLabel.Visible := True;
    Form2.Stage4MemoForWords.Enabled := False;
    Form2.Stage5MemoForWords.Enabled := True;
  End;
end;

procedure TForm2.Stage5GameTimerTimer(Sender: TObject);
Var
  GameString: String;
  I: Integer;
begin
  Stage5GameTimer.Enabled := False;
  Unit2.Path := Unit2.Path_Stage_2_3_4_5;

  if (Unit2.StageCounter = 5) And ((Unit2.PhaseCounter > 0) And (Unit2.PhaseCounter < 5))  then
  Begin
    Stage5GameWordsLabel.Caption := '';
    Stage5GameWordsLabel.Left := 64;
    GetGameWordsFromFileStage_2_3_4_5(Unit2.Path);

    for I := Low(Unit2.GameWords) to High(Unit2.GameWords) do
      GameString := GameString + Unit2.GameWords[I] + ' ';
    Stage5GameWordsLabel.Caption := GameString;
    ...
    Stage5GameWordsLabel.Caption := '';
    Delay(300);
    Stage5GameWordsLabel.Left := 40;
    Stage5GameWordsLabel.Font.Size := 8;
    Stage5GameWordsLabel.Caption := 'Вспомните слова и введите их все "наоборот" в поле ниже. Порядок важен!';
  End
  else
  Begin
    Form2.Stage5MemoForWords.Enabled := False;
    Application.MessageBox('Поздравляем с окончательной победой! Вы прошли все этапы!', 'Победа в пятом этапе!');

    FifthStage.TabVisible := False;
    FifthStage.Enabled := False;
    Form2.Stage5MemoForWords.Enabled := False;
    Delay(1000);
    Form2.Close;
  End;
...
procedure TForm2.Stage5MemoForWordsChange(Sender: TObject);
begin
  If(Stage5MemoForWords.Lines[0] <> '') then
  Begin
    Stage5ConfirmWordsButton.Enabled := True;
    Stage5MemoForWords.CharCase := ecUpperCase;
  End
  Else
  Begin
    Stage5ConfirmWordsButton.Enabled := False;
...
begin
  If Not(AnsiString(Key)[1] In ['А'..'Я','а'..'я','Ё','ё', #08, #45]) Then
    Key := #0;
end;

procedure TForm2.Stage2MemoForWordsKeyPress(Sender: TObject; var Key: Char);
begin
  If Not(AnsiString(Key)[1] In ['А'..'Я','а'..'я', 'Ё','ё', #08, #13, #45]) Then
    Key := #0;
end;
...
procedure TForm2.Stage5MemoForWordsKeyPress(Sender: TObject; var Key: Char);
begin
  If Not(AnsiString(Key)[1] In ['А'..'Я','а'..'я', 'Ё','ё', #08, #13, #45]) Then
    Key := #0;
end;
